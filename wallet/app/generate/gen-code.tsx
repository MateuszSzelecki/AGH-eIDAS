...
export default function CodeScreen(){
  const { user, biometricsCheck } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchCode = async () => {
      //TO DO: fetch data from real ZKP function
      try {
        const data = "123456";
        setCode(data);
      } catch (err) {
...
const authenticateAndGenerate = async () => {
    ...
    const result = await biometricsCheck('Generate verfication code');
...
<ThemedView style={styles.container}>
      <ThemedText style={styles.title}>Use this code to verify your age:</ThemedText>
